...
router.post('/', authMiddleware, async (req, res) => {
  ...
  if (text.length < 1)
    return res
      .status(401)
      .send('You must include at least one character in your text post.');
...
router.post('/like/:postId', authMiddleware, async (req, res) => {
  try {
    ...
    if (isLiked) {
      return res.status(401).send('This post has already been liked.');
    }
...
router.put('/unlike/:postId', authMiddleware, async (req, res) => {
  try {
    ...
    if (isLiked) {
      return res.status(401).send('This post has not been liked.');
    }
    const userIndex = post.likes
      .map((like) => like.user.toString())
      .indexOf(userId);

    await post.likes.splice(userIndex, 1);
